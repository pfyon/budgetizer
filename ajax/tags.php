	...
	switch($_POST['action'])
	{
		case "add":
			...
			if($tag_line != '')
			{
				...
				{
					$result = pg_query($cnx, "SELECT id FROM tag_labels WHERE label = '" . pg_escape_string($tagname) . "'");
					if(pg_num_rows($result) < 1)
					{
						//Gotta insert the tag so we get an ID first
						//TODO: using 1 as the owner for now
						$result = pg_query($cnx, "INSERT INTO tag_labels (label, owner) VALUES ('" . pg_escape_string($tagname) . "', 1) RETURNING id");
						if(pg_num_rows($result) == 1)
						{
					...
					if($tagid !== null)
					{
						pg_query($cnx, "INSERT INTO tag_list (tag, transaction_id) values (" . $tagid . ", " . pg_escape_string($transaction_id) . ")");
					}
				}
			}

			//And finally, we return the tags that are on the transaction
			$result = pg_query($cnx, "SELECT l.label FROM tag_labels l JOIN tag_list t ON l.id = t.tag AND t.transaction_id = " . pg_escape_string($transaction_id) . " ORDER BY l.label");
			while(($row = pg_fetch_assoc($result)) !== false)
			{
				$tags[] = $row['label'];
			}
		...
		case "remove":
			...
			Transactions::removeTag($tag, $transaction_id, $cnx);

			//Check to see if anything has this tag on it anymore
			$result = pg_query($cnx, "SELECT COUNT(i.id) FROM tag_list i JOIN tag_labels l ON i.tag = l.id WHERE l.label = '" . pg_escape_string($tag) . "'");
			if($result)
			{
				$row = pg_fetch_assoc($result, 0);
				if($row['count'] == '0')
				{
					...
					pg_query($cnx, "DELETE FROM tag_labels WHERE label = '" . pg_escape_string($tag) . "'");
